...
class SupervisorController extends Controller
{
    ...
    public function index()
    {
        ...
        $businessUnits = $this->bfrnApiClient->listBusinessUnits();
            ...
            if ($status === 'approved') {
                ...
                if ($identity) {
                    ...
                    if ($email) {
                        ...
                        $provisionedUser = $this->bfrnApiClient->provisionUser($email, $defaultBuId);

                        if (!$provisionedUser || empty($provisionedUser->user_id)) {
                            throw new \RuntimeException('BFRN user provisioning failed.');
                        }

                        $identity->bfrn_user_id = (int) $provisionedUser->user_id;
                        $temporaryPassword = $provisionedUser->temporary_password ?? null;
    ...
    private function handleEmailDecision(int $id, \Illuminate\Http\Request $request, string $status)
    {
        ...
        return $this->decide(
            ...
            $status,
            $reason,
            (int) (($approval->approved_bu_id ?? null) ?: env('BFRN_DEFAULT_BU_ID', 8))
        );
    }
